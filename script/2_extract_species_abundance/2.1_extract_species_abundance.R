...
#Standardise format
sngo <- sngo %>% 
  dplyr::mutate(species= "snow goose", zone= "study area", breeding_status= "breeding",method= "nest sampling") %>% 
  dplyr::select(species, zone, year, abundance, breeding_status, method)
sngo[sngo$year %in% c(2010:2019, 2022, 2023),]$method <- "combined methods"
sngo[sngo$year %in% c(1999:2009),]$method <- "nest sampling (extrapolation colony)"
...
#--------------------------------------------------#
#### 3.14 Nearctic Brown and collared lemmings ####
#-------------------------------------------------#
...
ermine <- ermine %>% 
  dplyr::filter(zone == "study area") %>% 
  dplyr::mutate(breeding_status= "undetermined", method= "testimonials") %>% 
  dplyr::select(-sd_ind_density_km2)
...
abundances_dataset <- abundances_dataset %>% 
  ...
  dplyr::select(species_en, year, breeding_status, abundance, method_description, method_quality)
