...
common_raven <- estimate_density_from_reference_species(
  ...
  dplyr::mutate(breeding_status= "undetermined", method= "transects (relative abundance)", year= NA, spatial_extrapolation= "yes") %>% 
...
common_raven_alternative <-relative_abundance_gauthier %>% 
  ...
  dplyr::mutate(breeding_status= "undetermined", method= "incidental observations (relative abundance)", year= NA, spatial_extrapolation= "yes") %>% 
  dplyr::select(species, zone, year, ind_density_km2, breeding_status, method, spatial_extrapolation)
