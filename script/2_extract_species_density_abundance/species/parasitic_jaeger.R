...
parasitic_jaeger <- data.frame(
  ...
  dplyr::mutate(breeding_status= "undetermined", method= "banding", year= NA, spatial_extrapolation= "no") %>% 
...
parasitic_jaeger_alternative <- get_nest_density(
  ...
  dplyr::mutate(breeding_status= "breeding", method= "nest sampling", year= NA, spatial_extrapolation= "no") %>% 
  dplyr::select(species, zone, year, ind_density_km2, breeding_status, method, spatial_extrapolation)
