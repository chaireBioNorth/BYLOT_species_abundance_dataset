...
black_bellied_plover_alternative <-relative_abundance_gauthier %>% 
  ...
  dplyr::mutate(ind_density_km2= ref_ind_density_km2*(index_sp/index_ref_sp), breeding_status= "breeding", method= "incidental observations (relative abundance)", year= NA, spatial_extrapolation= "no") %>% 
  dplyr::select(species, zone, year, ind_density_km2, breeding_status, method, spatial_extrapolation)
