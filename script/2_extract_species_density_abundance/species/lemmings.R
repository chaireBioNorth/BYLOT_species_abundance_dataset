...
dens_lemming_area_modified <- dens_lemming_area %>% 
  mutate(
    species = ifelse(species == "brown", "nearctic brown lemming", "nearctic collared lemming"),
...
dens_bl = data.frame(year = dens_lemming2$Year, zone = 'qarlikturvik valley', species = 'nearctic brown lemming', ind_density_km2 = dens_lemming2$Brown_density, sd_ind_density_km2 = NA)
dens_cl = data.frame(year = dens_lemming2$Year, zone = 'qarlikturvik valley', species = 'nearctic collared lemming', ind_density_km2 = dens_lemming2$Coll_density, sd_ind_density_km2 = NA)
...
lemming <- lemming %>% 
  dplyr::filter(zone =="study area") %>% 
  dplyr::mutate(breeding_status= "undetermined", method= "trapping", spatial_extrapolation= "yes") %>% 
