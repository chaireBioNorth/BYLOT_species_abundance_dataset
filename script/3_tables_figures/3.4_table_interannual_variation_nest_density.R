...
nest_density_table <- nest_density_study_area %>% 
  ...
  dplyr::summarise(mean_nest_density_km2= format(round(mean(nest_density_km2), digits=3),nsmall = 3), sd_nest_density_km2= format(round(sd(nest_density_km2), digits=3), nsmall=3), number_years= n(), .groups = "drop")

#--- Export as .csv for the data set
nest_density_table <- nest_density_table %>% 
  dplyr::left_join(sp_taxonomy)

nest_density_table_df <- nest_density_table %>% 
  dplyr::select(species_en, zone, mean_nest_density_km2, sd_nest_density_km2, number_years) %>% 
  dplyr::arrange(zone)

write.csv(nest_density_table_df,"DataS1/BYLOT-interannual_variation_nest_density.csv",row.names = FALSE)
...
table <- nest_density_table %>% 
  ...
  dplyr::rename(Species= species_en, Zone= zone, `Mean nests/km2 ± SD`= mean_nest_density_km2, `Number of years`= number_years) %>% 
  ...
  dplyr::arrange(Zone, `Mean nests/km2 ± SD`, Species) %>% 
  dplyr::select(Species, Zone, `Mean nests/km2 ± SD`, `Number of years`)
...
print(table_latex,
      ...
      file = "MetadataS1/tables/table_interannual_nest_density_variation.tex",
